...
Recursively call passing the height as an argument, check the left and right branches increasing the height until the leaves of the tree. Upon return, check the max height on the left and right, if the difference is bigger than 1 return a negative flag. Complexity: O(n)
